...
eval_loader = DataLoader( eval_set, batch_size= batch_size, shuffle=False  ,pin_memory=True)
    ...
    def __call__(self, predicts,y ,seq_len ):
        ...
        for i,num in enumerate( seq_len ):
            ...
            if torch.equal( predicts[i][:num], y[i][:num] ):
                hit += 1
            # for seqeval use
            self.total_predict.append( predicts[i][:num].tolist() )
            self.total_ground_truth.append( y[i][:num].tolist() )
        ...
        return loss
    def show_seqeval(self,idx2tag):
        y_pred = [ [ idx2tag[idx] for idx in seq] \
                  for seq in self.total_predict ]
        y_true = [ [ idx2tag[idx] for idx in seq] \
                  for seq in self.total_ground_truth ]
        print( classification_report( y_pred, y_true, mode="strict",scheme=IOB2) ) 
...
def save_model(model, model_name= "slot_model.ckpt"):
...
def train(model,dataloader, epoch):
    ...
    for (x,seq_len),y in pbar:
        ...
        optimizer.step()
    metrics.show_score( epoch )
    # save_model( model , f"E-{epoch}-{get_now_time_str()}-SeqClassifier.ckpt"  )
    # print( f"epoch: {epoch} loss:{loss} accuracy:{accuracy}" )

def load_model(model, model_file = "slot.ckpt"):
...
def eval(model,dataloader,epoch):
    ...
    metrics.show_score( epoch  )
    metrics.show_seqeval( eval_set.idx2tag )
    print(  )
